...
{
	pr_alert("USB KEYBOARD PLUGGED IN"); /* KB DETECTED */
	return 0;
}

static void usbmon_disconnect(struct usb_interface *intf)
{
	pr_alert("USB KEYBOARD DISCONNECTED"); /* DISCONNECTION KB */
}

static struct usb_driver usbmon_toy_driver = { .name = "usbmon_toy",
					       .probe = usbmon_probe,
